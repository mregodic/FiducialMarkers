...
def plot_histogram(pos_features,neg_features,pos_labels='Knowns',neg_labels='Unknowns',title="Histogram",file_name='{}foo.pdf'):
    ...
    pos_hist=np.histogram(pos_mag, bins='auto') #500
    neg_hist=np.histogram(neg_mag, bins='auto') #500
    ...
    plt.xscale('log') # MR: disabling log scale
    ax.get_xaxis().set_ticks([1, 10, 100,]) #,100,1000,10000
...
def plot_softmax_histogram(pos_prob,neg_prob,pos_labels='Knowns',neg_labels='Unknowns',title=None,file_name='{}foo.pdf'):    
    ...
    num_bins_pos = 50
    num_bins_neg = 50
    if(len(pos) < 50): num_bins_pos = 3
    if(len(neg) < 50): num_bins_neg = 3
    
     
    pos_hist=np.histogram(pos, bins=num_bins_pos, range = (0, 1))#'auto') #50 #, range = (0, 1)
    neg_hist=np.histogram(neg, bins=num_bins_neg, range = (0, 1))#'auto') $50
    ...
    plt.yscale('log') # mr disabled
    ax.set_xticks(np.arange(0, 1.1, .1))
    ax.get_yaxis().set_ticks([10, 100, 1000]) #,100,1000,10000
    #ax.set_yticks(np.arange(0, 20, 2)) #,100,1000,10000
...
def _plot_softmax_histogram(pos_prob,neg_prob,pos_labels='Knowns',neg_labels='Unknowns',title=None,file_name='{}foo.pdf'):    
    if pos_prob.shape[1]==11:
        ...
        pos=np.max(pos_prob,axis=1)
        neg=np.max(neg_prob,axis=1)
    pos_hist=np.histogram(pos, bins='auto') #50
    neg_hist=np.histogram(neg, bins='auto') #50
